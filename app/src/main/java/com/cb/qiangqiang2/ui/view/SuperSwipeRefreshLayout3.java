...
public class SuperSwipeRefreshLayout3 extends ViewGroup {
    ...
    // SuperSwipeRefreshLayout3内的目标View，比如RecyclerView,ListView,ScrollView,GridView
    // etc.
    ...
    public SuperSwipeRefreshLayout3(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    protected void onLayout(boolean changed, int left, int top, int right,
        ...
        Timber.d(LOG_TAG, "debug:onLayout childHeight = " + childHeight);
    ...
    private boolean handlerPushTouchEvent(MotionEvent ev, int action) {
        switch (action) {
            case MotionEvent.ACTION_DOWN:
                mActivePointerId = MotionEventCompat.getPointerId(ev, 0);
                mIsBeingDragged = false;
                Timber.d(LOG_TAG, "debug:onTouchEvent ACTION_DOWN");
                break;
            case MotionEvent.ACTION_MOVE: {
                final int pointerIndex = MotionEventCompat.findPointerIndex(ev,
                        mActivePointerId);
